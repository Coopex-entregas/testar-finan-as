...
def update_config():
    cfg = get_config()
    if "salario_minimo" in request.form:
...
def delete_cooperado(id):
    ...
    try:
        ...
        if escala_ids:
            db.session.execute(
                sa_delete(TrocaSolicitacao).where(TrocaSolicitacao.origem_escala_id.in_(escala_ids))
            )
            db.session.execute(
                sa_delete(Escala).where(Escala.id.in_(escala_ids))
            )
